...
**分段学习建议（碎片时间适用）：**
- 资源1 可以分 3 次看，不用一次看完：
  - 第 1 次：整体结构 + 构造方法 + add() 流程
  - 第 2 次：grow() 扩容 + 1.5 倍计算
  - 第 3 次：remove() 元素前移 + get() + fail-fast
- 每次结束时，在笔记里写：这次学到哪、下次从哪继续
- 不要因为“没一次看完”而焦虑，任务允许分多次完成
...
> 目标是屏幕前的你写出能用的简化版，比如 add / get / remove / 扩容。
